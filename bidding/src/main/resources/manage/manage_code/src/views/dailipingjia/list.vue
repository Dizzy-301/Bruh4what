<template>
	<div>
		<div class="center_view">
			...
			<br />
			<el-table :data="list" border stripe v-loading="listLoading" style="width: 100%" v-if="btnAuth('dailipingjia','查看')">
				...
				<el-table-column prop="dailigongsimingcheng" label="代理公司" align="left" header-align="left"></el-table-column>
				<el-table-column label="满意度" align="left" header-align="left">
					<template #default="scope">
						<el-tag :type="scope.row.manyidu >= 9 ? 'success' : scope.row.manyidu >= 8 ? 'warning' : 'info'">
							{{scope.row.manyidu?Number(scope.row.manyidu).toFixed(1):'0.0'}} 分
						</el-tag>
					</template>
				</el-table-column>
				<el-table-column prop="yezhuzhanghao" label="评价账号" align="left" header-align="left"></el-table-column>
				<el-table-column prop="yezhumingcheng" label="评价人" align="left" header-align="left"></el-table-column>
				<el-table-column prop="pingjianeirong" label="评价内容" align="left" header-align="left" show-overflow-tooltip></el-table-column>
				<el-table-column prop="addtime" label="评价时间" align="left" header-align="left" width="180"></el-table-column>
				<el-table-column label="操作" align="left" header-align="left" width="200">
					<template #default="scope">
						<el-button type="primary" size="small" @click="infoClick(scope.row.id)" v-if="btnAuth('dailipingjia','查看')">详情</el-button>
						<el-button type="danger" size="small" @click="deleteClick(scope.row.id)" v-if="btnAuth('dailipingjia','删除')">删除</el-button>
					</template>
				</el-table-column>
			</el-table>
...
const deleteClick = (id) => {
	context.$confirm('确定删除该评价吗？', '提示', {
		confirmButtonText: '确定',
		cancelButtonText: '取消',
		type: 'warning'
	}).then(() => {
		context.$http({
			url: 'dailipingjia/delete',
			method: 'post',
			data: [id]
		}).then(() => {
