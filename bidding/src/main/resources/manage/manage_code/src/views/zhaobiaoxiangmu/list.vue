    ...
    <div>
        <div class="center_view">
            <div class="list_search_view">
                ...
                <div class="btn_view">
                    <el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('zhaobiaoxiangmu','新增')">
                        新增
                    </el-button>
                </div>
            </div>
            <div class="dataList"
                 v-if="btnAuth('zhaobiaoxiangmu','查看')"
            >
                <div class="item" v-for="(item,index) in list" :key="item.id">
                    ...
                    <div class="title">项目名称：{{item.xiangmumingcheng}}</div>
                    <div class="title">项目预算：{{item.xiangmuyusuan}}</div>
                    <div class="title">企业账号：{{item.qiyezhanghao}}</div>
                    ...
                    <div class="btns">
                        <el-button class="view_btn" type="info" v-if=" btnAuth('zhaobiaoxiangmu','查看')"
                                   @click="infoClick(item.id)">
                            详情
                        </el-button>
                        <el-button class="edit_btn" type="primary" @click="editClick(item.id)"
                                   v-if=" btnAuth('zhaobiaoxiangmu','修改')">
                            修改
                        </el-button>
                        <el-button class="del_btn" type="danger" @click="delClick(item.id)"
                                   v-if="btnAuth('zhaobiaoxiangmu','删除')">
                            删除
                        </el-button>
                        <el-button class="cross_btn" v-if="btnAuth('zhaobiaoxiangmu','投标')" type="success"
                                   @click="toubiaoxinxiCrossAddOrUpdateHandler(item,'cross','否','','','')">
                            投标
                        </el-button>
                        <el-button class="operate_btn" v-if="item.sfsh!='是' && btnAuth('zhaobiaoxiangmu','审核')"
                                   @click="approvalClick(item)" type="warning">审核
                        </el-button>
                    </div>
                </div>
            </div>
        ...
        <toubiaoxinxiFormModel ref="toubiaoxinxiFormModelRef"
                               @formModelChange="formModelChange"></toubiaoxinxiFormModel>
    </div>
</template>
<script setup>
    ...
    const approvalClick = (row) => {
        let params = {
            ...
            zhaobiaofanwei: row.zhaobiaofanwei,
            zhaobiaowenjian: row.zhaobiaowenjian,
            toubiaojiezhishijian: row.toubiaojiezhishijian,
            kaibiaoshijian: row.kaibiaoshijian,
            xiangmudidian: row.xiangmudidian,
            xiangmuzhouqi: row.xiangmuzhouqi,
            xiangmuyusuan: row.xiangmuyusuan,
            hetongtiaokuan: row.hetongtiaokuan,
            sfsh: row.sfsh,
            shhf: row.shhf,
            fengmian: row.fengmian,
            dailigongsizhanghao: row.dailigongsizhanghao,
            dailigongsimingcheng: row.dailigongsimingcheng,
        }
        nextTick(() => {
            approvalRef.value.approvalClick(params)
        })
    }
    import toubiaoxinxiFormModel from '@/views/toubiaoxinxi/formModel'

    const toubiaoxinxiFormModelRef = ref(null)
    const toubiaoxinxiCrossAddOrUpdateHandler = (row, type, crossOptAudit, crossOptPay, statusColumnName, tips, statusColumnValue) => {
        ...
        nextTick(() => {
            toubiaoxinxiFormModelRef.value.init(row.id, 'cross', '投标', row, 'zhaobiaoxiangmu', statusColumnName, tips, statusColumnValue)
        })
    }
    //查询审核状态列表
