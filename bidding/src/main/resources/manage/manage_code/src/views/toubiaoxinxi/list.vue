    ...
    <div>
        <div class="center_view">
            <div class="list_search_view">
                ...
                <div class="btn_view">
                    <el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('toubiaoxinxi','新增')">
                        新增
                    </el-button>
                    <el-button class="del_btn" type="danger" :disabled="selRows.length?false:true"
                               @click="delClick(null)" v-if="btnAuth('toubiaoxinxi','删除')">
                        删除
                    </el-button>
                </div>
            </div>
            <el-table
                ...
                <el-table-column label="资质证书" min-width="140" :resizable='true' :sortable='true' align="left"
                                 header-align="left">
                    <template #default="scope">
                        <el-button v-if="scope.row.zizhizhengshu" link size="small"
                                   @click="download(scope.row.zizhizhengshu)">下载
                        </el-button>
                ...
                <el-table-column min-width="140"
                                 ...
                                 align="left"
                                 header-align="left"
                                 prop="toubiaozhanghao"
                                 label="投标账号">
                    <template #default="scope">
                        {{scope.row.toubiaozhanghao}}
                    </template>
                </el-table-column>
                <el-table-column min-width="140"
                                 :resizable='true'
                ...
                <el-table-column min-width="140"
                                 ...
                                 align="left"
                                 header-align="left"
                                 prop="toubiaozongbaojia"
                                 label="投标总报价">
                    <template #default="scope">
                        {{scope.row.toubiaozongbaojia}}
                    </template>
                </el-table-column>
                <el-table-column min-width="140"
                                 :resizable='true'
                ...
                <el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left"
                                 header-align="left">
                    <template #default="scope">
                        <el-button class="view_btn" type="info" v-if=" btnAuth('toubiaoxinxi','查看')"
                                   @click="infoClick(scope.row.id)">
                            详情
                        </el-button>
                        <el-button class="edit_btn" type="primary" @click="editClick(scope.row.id)"
                                   v-if=" btnAuth('toubiaoxinxi','修改')">
                            修改
                        </el-button>
                        <el-button class="del_btn" type="danger" @click="delClick(scope.row.id)"
                                   v-if="btnAuth('toubiaoxinxi','删除')">
                            删除
                        </el-button>
                        <el-button class="cross_btn" v-if="btnAuth('toubiaoxinxi','中标公告')" type="success"
                                   @click="zhongbiaogonggaoCrossAddOrUpdateHandler(scope.row,'cross','','','','')">
                            中标公告
                        </el-button>
                        <el-button class="resume_btn" type="warning" @click="viewResume(scope.row)"
                                   v-if="btnAuth('toubiaoxinxi','查看履历')">
                            查看履历
                        </el-button>
                    </template>
                </el-table-column>
        ...
        <zhongbiaogonggaoFormModel ref="zhongbiaogonggaoFormModelRef"
                                   @formModelChange="formModelChange"></zhongbiaogonggaoFormModel>
...
<script setup>
    ...
    import zhongbiaogonggaoFormModel from '@/views/zhongbiaogonggao/formModel'

    const zhongbiaogonggaoFormModelRef = ref(null)
    const zhongbiaogonggaoCrossAddOrUpdateHandler = (row, type, crossOptAudit, crossOptPay, statusColumnName, tips, statusColumnValue) => {
        ...
        nextTick(() => {
            zhongbiaogonggaoFormModelRef.value.init(row.id, 'cross', '中标公告', row, 'toubiaoxinxi', statusColumnName, tips, statusColumnValue)
        })
    }
    // 履历
    ...
    const viewResume = (row) => {
        context.$http({
            ...
            params: {
                page: 1,
                limit: 100,
                toubiaorenzhanghao: row.toubiaozhanghao
            }
        }).then(res => {
