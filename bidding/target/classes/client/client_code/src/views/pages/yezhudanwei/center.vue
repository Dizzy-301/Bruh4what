    ...
    <div class="center_view" :style='{}'>
        ...
        <div class="usersView">
            <div class="usersTabView">
                <div class="usersTab" :class="tabIndex=='center'?'usersTabActive':''"
                     @click="tabClick({tableName:'center'})">个人中心
                </div>
                <div class="usersTab " :class="tabIndex=='updatePassword'?'usersTabActive':''"
                     @click="tabClick({tableName:'updatePassword'})">修改密码
                </div>
                    ...
                    <div v-if="item.child.length>1" class="usersTab" @mouseenter="usersTabHover(index)"
                        ...
                        <el-collapse-transition>
                            <div class="usersTabHoverView" v-if="usersTabIndex==index">
                                <div class="usersTabHoverTab" v-for="(items,indexs) in item.child"
                    ...
                    <el-row>
                        <el-col :span="12">
                            <el-form-item prop="qiyezhanghao" label="企业账号">
                                <el-input class="list_inp" v-model="userForm.qiyezhanghao" placeholder="企业账号"
                                          readonly></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item prop="qiyemingcheng" label="企业名称">
                                <el-input class="list_inp" v-model="userForm.qiyemingcheng"
                                          placeholder="企业名称"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item prop="fuzeren" label="负责人">
                                <el-input class="list_inp" v-model="userForm.fuzeren" placeholder="负责人"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item prop="lianxidianhua" label="联系电话">
                                <el-input class="list_inp" v-model="userForm.lianxidianhua"
                        ...
                        <el-col :span="24">
                            <el-form-item prop="zizhi" label="资质">
                                <uploads
                                        action="file/upload"
                                        tip="请上传资质"
                                        style="width: 100%;text-align: left;"
                                        :fileUrls="userForm.zizhi?userForm.zizhi:''"
...
<script setup>
    ...
    const role = ref('')
    //资质上传回调
    const zizhiUploadSuccess = (e) => {
        userForm.value.zizhi = e
    }
    //初始化
    const init = () => {
        ...
        if (arr) {
            arr = arr.map(item => {
                if (item.child) {
                    item.child = item.child.filter(c => c.tableName !== 'dailipingjia')
                }
                return item
    ...
    const updateSession = () => {
        userFormRef.value.validate((valid) => {
            if (valid) {
                if (userForm.value.zizhi != null) {
                    userForm.value.zizhi = userForm.value.zizhi.replace(new RegExp(context?.$config.url, "g"), "");
                }
                store.dispatch('user/update', userForm.value).then(res => {
                    if (res?.data && res.data.code == 0) {
                        context?.$toolUtil.message('更新成功', 'success')
